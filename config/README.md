...
| Path | Contents |
| --- | --- |
| `shroudforge/config/shroudforge.json` | Loader, logging, and module settings |
| `shroudforge/version.json` | Release version and build metadata, generated for the player ZIP |
...
same installation-wide lock at `shroudforge/config/.shroudforge-write.lock`;
mod packages do not contain lock files.

The loader generates `shroudforge/config/state.json` as needed. It combines update results,
...
ZIP. `shroudforge/config/news/news.json` is embedded into the loader binary at build time
...
`shroudforge/config/shroudforge.json`, `shroudforge/config/state.json`, and mod manifests with their
