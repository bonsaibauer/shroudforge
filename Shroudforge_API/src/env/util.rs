...
use kfc::{
    ...
    reflection::{LookupKey, TypeHandle, TypeRegistry},
};
use mlua::ObjectLike;
use mod_loader::Mod;
...
use crate::{
    ...
    lua::{Args, CastLua, CastLuaExt, Either, FunctionArgs, LuaError, LuaValue},
};

pub fn get_type<T>(args: &impl Args, index: usize, type_registry: T) -> mlua::Result<TypeHandle<T>>
where
    T: Borrow<TypeRegistry> + Clone,
{
    let r#type = args.get::<Either<String, &Type>>(index)?;
    let r#type = match r#type {
        Either::A(type_name) => type_registry
            .borrow()
        ...
        Either::B(type_data) => TypeHandle::new(type_registry, type_data.index),
    };
    ...
    R: mlua::IntoLuaMulti,
{
    ...
    let function =
        lua.create_function(move |lua, args: FunctionArgs| func(lua, args, &mod_clone))?;
...
impl<'a> CastLua<'a> for Guid {
    ...
    fn try_cast_lua(value: &'a LuaValue) -> mlua::Result<Option<Self::Output>> {
        match value {
            LuaValue::String(s) => {
                let s = s.to_str()?;
