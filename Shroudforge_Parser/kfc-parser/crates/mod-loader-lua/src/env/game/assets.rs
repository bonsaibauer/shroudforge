use kfc::{
    guid::{ContentHash, Guid, ResourceId},
    reflection::LookupKey,
};
use mlua::Table;
use tracing::warn;
...
fn lua_get_resource(lua: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Option<Resource>> {
    ...
    let guid = ResourceId::new(guid, r#type.qualified_hash, part.unwrap_or(0));
...
fn lua_get_resource_parts(lua: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Table> {
    ...
    let target_guid = ResourceId::new(guid, r#type.qualified_hash, 0);
...
fn lua_create_resource(lua: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Resource> {
    ...
    let guid = if args.len() > 2 {
        ...
        let guid = ResourceId::new(guid, r#type.qualified_hash, part);
