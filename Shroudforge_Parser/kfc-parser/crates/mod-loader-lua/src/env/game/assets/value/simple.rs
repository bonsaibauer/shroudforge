...
use crate::{
    alias::TypeHandle,
    env::value::{
        converter::{Converter, LuaConversionErrorKind},
...
impl StructValue {
    ...
    #[inline]
    pub fn new_with_data(r#type: TypeHandle, map: HashMap<String, LuaValue>) -> mlua::Result<Self> {
        Ok(Self {
            r#type,
    ...
    pub fn convert(&self, converter: &mut Converter) -> Result<Value, LuaConversionErrorKind> {
        ...
        for (key, value) in fields.iter() {
            ...
            let converted_value = converter.process(value, &field_type)?;
    ...
    pub fn try_clone_lua(&self, lua: &mlua::Lua) -> mlua::Result<Self> {
        ...
        for (key, value) in table.iter() {
            fields.insert(key.clone(), try_clone_lua_value(value, lua)?);
        }
    ...
    pub fn get(&self, key: String) -> mlua::Result<LuaValue> {
        Ok(self
            .fields
            .borrow()
            .get(&key)
    ...
    pub fn set(&self, key: String, value: &LuaValue, lua: &mlua::Lua) -> mlua::Result<()> {
        ...
        let value =
            validate_and_clone_lua_value(value, &self.r#type, lua).map_err(LuaError::external)?;
...
impl StructValueIter {
    fn create(
        ...
        let state = Self::new(value.r#type.clone(), value.fields.clone(), |r#type| {
            let iter = r#type.iter_fields().map(|field| field.name.as_str());
    ...
    fn next(&self, lua: &mlua::Lua) -> mlua::Result<(LuaValue, LuaValue)> {
        ...
        let value =
            self.with_fields(|fields| fields.borrow().get(key).cloned().unwrap_or(LuaValue::Nil));
...
impl ArrayValue {
    ...
    /// otherwise it will lead to unexpected behavior.
    #[inline]
    pub fn new_with_data(r#type: TypeHandle, data: Vec<LuaValue>) -> mlua::Result<Self> {
        Ok(Self {
            inner_type: r#type
                .inner_type()
    ...
    pub fn get(&self, index: usize) -> mlua::Result<LuaValue> {
        if index == 0 {
    ...
    pub fn remove(&mut self, index: usize) -> mlua::Result<Option<LuaValue>> {
        if index == 0 {
            return Ok(None);
        }
...
impl ArrayValueIter {
    ...
    fn next(&mut self) -> mlua::Result<(LuaValue, LuaValue)> {
...
impl VariantValue {
    ...
    pub fn convert(&self, converter: &mut Converter) -> Result<Value, LuaConversionErrorKind> {
        let value = converter
            .process(&self.value, &self.variant_type)?
            .into_struct()
