...
use kfc::{
    ...
    resource::{mapped::MappingError, value::Value},
};
...
impl Converter {
    pub fn convert(value: &LuaValue, r#type: &TypeHandle) -> Result<Value, LuaConversionError> {
    ...
    pub fn process(
        ...
        let lua_value = match r#type.primitive_type {
            ...
            PrimitiveType::SInt8 => self
                .convert_sint(value, i8::MIN as i64, i8::MAX as i64)?
                .into(),
            PrimitiveType::UInt16 => self.convert_uint(value, 0, u16::MAX as u64)?.into(),
            PrimitiveType::SInt16 => self
                .convert_sint(value, i16::MIN as i64, i16::MAX as i64)?
                .into(),
            PrimitiveType::UInt32 => self.convert_uint(value, 0, u32::MAX as u64)?.into(),
            PrimitiveType::SInt32 => self
                .convert_sint(value, i32::MIN as i64, i32::MAX as i64)?
                .into(),
    ...
    #[inline]
    fn convert_nil(&mut self, value: &LuaValue) -> Result<Value, LuaConversionErrorKind> {
        match value {
    ...
    #[inline]
    fn convert_bool(&mut self, value: &LuaValue) -> Result<bool, LuaConversionErrorKind> {
        match value {
    ...
    #[inline]
    fn convert_float(&mut self, value: &LuaValue) -> Result<f64, LuaConversionErrorKind> {
        match value {
    ...
    #[inline]
    fn convert_blob_string(&mut self, value: &LuaValue) -> Result<Value, LuaConversionErrorKind> {
        match value {
    ...
    fn convert_blob_variant(
        ...
        match value {
            LuaValue::UserData(ud) => self.try_convert_variant_ud(ud, r#type),
            _ => Err(LuaConversionErrorKind::IncompatibleType {
                expected: name_of(r#type),
                found: value.to_string()?,
            }),
    ...
    fn convert_object_reference(
        ...
        match value {
            LuaValue::String(s) => {
                ...
                let guid =
                    Guid::parse(&guid).ok_or_else(|| LuaConversionErrorKind::InvalidGuid(guid))?;
    ...
    #[inline]
    fn convert_guid(&mut self, value: &LuaValue) -> Result<Value, LuaConversionErrorKind> {
        match value {
            LuaValue::String(s) => {
                let guid = s.to_string_lossy();
                let guid =
...
pub fn value_to_lua(value: &MappedValue, lua: &mlua::Lua) -> mlua::Result<LuaValue> {
    let lua_value = match value {
        ...
        MappedValue::Bitmask(v) => {
            let wrapper = ArrayValue::new_with_data(
                v.r#type().clone(),
                v.iter()
                    .map(|bit| match bit.name() {
                        Some(name) => lua.create_string(name).map(LuaValue::String),
                        None => lua
                            .create_string(bit.value().to_string())
                            .map(LuaValue::String),
                    })
        ...
        MappedValue::String(string_value) => string_value
            .as_str()
        ...
        MappedValue::Guid(guid) => {
            ...
            lua.create_string(&guid_str).map(LuaValue::String)?
        }
