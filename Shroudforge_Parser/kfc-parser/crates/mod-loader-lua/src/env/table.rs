...
use crate::{
    env::value::{
        clear as clear_ud, insert as insert_ud, pop as pop_ud, push as push_ud, remove as remove_ud,
    },
    lua::{Either, FunctionArgs, LuaValue},
};

#[allow(clippy::get_first)]
pub fn modify(table: &mlua::Table, lua: &mlua::Lua) -> mlua::Result<()> {
    ...
    table.set(
        "insert",
        lua.create_function(move |lua, args: FunctionArgs| match args.len() {
            2 => {
    ...
    table.set(
        "clear",
        lua.create_function(move |_, args: FunctionArgs| {
