...
use crate::{
    env::{buffer::Buffer, util::add_function},
...
fn lua_create(_: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Image> {
...
fn lua_decode(_: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Image> {
    ...
    let image = match format {
        Some(format) => image::load_from_memory_with_format(buffer, format)
            .map_err(|e| LuaError::generic(format!("failed to decode image: {e}",)))?
            .to_rgba8(),
...
fn lua_decode_texture(_: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Image> {
    let buffer = args.get::<Either<&[u8], &Buffer>>(0)?;
    let buffer = match &buffer {
        Either::A(slice) => slice.as_ref(),
        Either::B(buf) => buf.data()?,
    ...
    kfc::content::image::decode(format, width as usize, height as usize, buffer, &mut dst).unwrap();

    Image::from_vec(dst, width, height)
}

fn lua_encode(_: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Buffer> {
    let image = args.get::<&Image>(0)?;
    let format = args
        .get::<Option<ImageFormat>>(1)?
    ...
    image
        ...
        .map_err(|e| LuaError::generic(format!("failed to encode image: {e}",)))?;

    Ok(Buffer::wrap(encoded.into_inner()))
}

fn lua_encode_texture(_: &mlua::Lua, args: FunctionArgs) -> mlua::Result<Buffer> {
    let image = args.get::<&Image>(0)?;
    let format = args.get::<PixelFormat>(1)?;
    ...
    let required_bytes = kfc::content::image::size_of_format(format, width, height);
    let mut encoded = vec![0u8; required_bytes];

    kfc::content::image::encode(format, width, height, image.inner(), &mut encoded)
        .map_err(|e| LuaError::generic(format!("failed to encode image: {e}",)))?;
