...
use mod_loader::{
    Capability, ModEnvironment, ModManifest,
    lua::{RunArgs, RunOptions, export_lua_definitions},
};
use semver::Version;
...
fn resolve_file_name(game_directory: &Path, file_name: Option<String>) -> String {
...
fn create(
    ...
    let capability_list = ["Patch", "Export", "Runtime"];
    ...
    std::fs::write(mod_dir.join(".luarc.json"), LUARC_JSON)?;
    ...
    std::fs::write(src_dir.join("mod.lua"), DEFAULT_MOD)?;

    match game_directory.to_str() {
        Some(path) => {
            export_lua_definitions(path, &file_name, false);
        }
        None => {
        ...
        Err(e) => {
            ...
            for mod_error in e.mods {
                if let Some(id) = &mod_error.id {
                    error!(
                        "  In mod '{}' at {}: {}",
                        id, mod_error.path, mod_error.error
                    );
                } else {
                    error!("  In mod at {}: {}", mod_error.path, mod_error.error);
                }
            }
...
fn restore(
    ...
    mod_loader::lua::restore(game_directory.to_str().unwrap(), &file_name);
