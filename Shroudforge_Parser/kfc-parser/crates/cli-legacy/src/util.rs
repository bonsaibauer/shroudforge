...
use kfc::{
    container::{KFCCursor, KFCReader},
    guid::ResourceId,
    reflection::{LookupKey, TypeRegistry},
    resource::value::{ConversionOptions, Value},
};
    ...
    R: Borrow<KFCReader>,
{
    ...
    let result = deserialize_descriptor(type_registry, guid, buf)?;
...
pub fn serialize_descriptor(
    ...
    serialize_descriptor_into(type_registry, value, &mut result).map(|guid| (guid, result))
}
...
pub fn serialize_descriptor_into(
    ...
    if let Some(obj) = value.as_struct() {
        ...
        let part = obj.get("$part").and_then(|v| v.as_u64()).unwrap_or(0) as u32;

        let r#type = type_registry
            .get_by_name(LookupKey::Qualified(type_name))
            .ok_or_else(|| anyhow::anyhow!("Invalid type: {}", type_name))?;
        let guid = ResourceId::parse(guid, r#type.qualified_hash, part)
            .ok_or_else(|| anyhow::anyhow!("Invalid GUID: {}", guid))?;

        value.write_into(type_registry, r#type, dst)?;
...
pub fn deserialize_descriptor(
    ...
    let r#type = type_registry
        .get_by_hash(LookupKey::Qualified(guid.type_hash()))
