...
fn main() {
    ...
    let result = match cli.commands {
        ...
        Commands::Restore {
            game_directory,
            file_name,
        } => revert_repack(&game_directory, file_name.as_deref(), false),
        Commands::Impact(impact) => match impact {
            ...
            CommandImpact::Disassemble { input, output } => {
                disassemble_impact(&input, output.as_deref())
            }
            CommandImpact::ExtractNodes => extract_nodes(),
        },
    };
...
fn unpack(
    ...
    if let Some(output_dir) = output_dir {
        ...
        unpack_files(&kfc_reader, &type_registry, output_dir, guids, thread_count)
    } else if stdout {
        unpack_stdout(&kfc_reader, &type_registry, guids, thread_count)
    } else {
...
fn unpack_files(
    ...
    pb.set_style(
        ProgressStyle::default_bar()
            .template(&format!(
                "{} Unpacking... [{{bar:40}}] {{pos:>7}}/{{len:7}} {{msg}}",
                "info:".blue().bold()
            ))
            .unwrap()
            .progress_chars("##-"),
...
fn validate_backup(kfc_path: &Path, kfc_path_bak: &Path) -> Result<bool, Error> {
...
fn repack_files(
    ...
    let mut writer =
        match KFCWriter::new_incremental(game_dir, &file_name, ref_kfc_file, type_registry) {
            Ok(writer) => writer,
            Err(e) => fatal!("Failed to open {}: {}", file_path.display(), e),
        };
    ...
    pb_serialize.set_style(
        ProgressStyle::default_bar()
            .template(&format!(
                "{} Serializing... [{{bar:40}}] {{pos:>7}}/{{len:7}} {{msg}}",
                "info:".blue().bold()
            ))
            .unwrap()
            .progress_chars("##-"),
    ...
    pb_write.set_style(
        ProgressStyle::default_bar()
            .template(&format!(
                "{} Writing... [{{bar:40}}] {{pos:>7}}/{{len:7}} {{msg}}",
                "info:".blue().bold()
            ))
            .unwrap()
            .progress_chars("##-"),
    ...
    info!(
        "Repacked a total of {}/{} descriptors in {:?}",
        total - failed_repacks,
        total,
        end - start
    );
    ...
    match std::fs::copy(&kfc_file_bak, &kfc_file) {
        ...
        Err(e) => {
            ...
            if kfc_file_bak.exists() {
                fatal!(
                    "Please manually restore from backup file: {}",
                    kfc_file_bak.display()
                );
            } else {
...
fn extract_types(game_dir: &Path, file_name: Option<&str>) -> Result<(), Error> {
        ...
        Ok(type_registry) => {
            if let Some(game_dir) = game_dir {
                ...
                if type_registry.version != version_tag {
                    if retry {
                        warn!(
                            "reflection_data.json is outdated, attempting to extract types again..."
                        );
...
fn assemble_impact(
    ...
    info!(
        " - {} data entries ({} bytes)",
        impact_program.data_layout.len(),
        impact_program.data.len()
    );

    // write to file

    let output_file = output_file.unwrap_or(input_file).with_extension("json");
    ...
    info!(
        "Impact program has been written to {}",
        output_file.display()
    );
...
fn disassemble_impact(input_file: &Path, output_file: Option<&Path>) -> Result<(), Error> {
    let type_registry = load_type_registry(None, None, true)?;
...
fn extract_nodes() -> Result<(), Error> {
    let type_registry = load_type_registry(None, None, true)?;
    let nodes = type_registry
        .get_impact_nodes()
        .into_values()
        .collect::<Vec<_>>();

    let output_path = current_exe()
        .unwrap()
        .parent()
        .unwrap()
    ...
    info!(
        "Impact node data has been written to {}",
        std::path::absolute(output_path).unwrap().display()
    );

    Ok(())
}

fn get_file(game_dir: &Path, file_name: Option<&str>, extension: &str) -> Result<PathBuf, Error> {
    let file_name = get_file_name(game_dir, file_name)?;
...
fn get_file_name(game_dir: &Path, file_name: Option<&str>) -> Result<String, Error> {
    if let Some(file_name) = file_name {
        Ok(file_name.into())
    } else if game_dir.join("enshrouded.kfc").exists() || game_dir.join("enshrouded.exe").exists() {
        Ok("enshrouded".into())
    } else if game_dir.join("enshrouded_server.kfc").exists()
        || game_dir.join("enshrouded_server.exe").exists()
    {
        ...
        fatal!(
            "Unable to find the kfc/exe file in game directory, please specify it with --file_name"
        );
    }
}

fn load_types_from_path(path: impl AsRef<Path>) -> Result<TypeRegistry, TypeParseError> {
